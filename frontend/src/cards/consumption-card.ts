...
export interface ConsumptionCardConfig {
  type: string
  meter_type?: string
  meter_serial?: string
}

class EonNextConsumptionCard extends LitElement {
  ...
  @state() private _config!: ConsumptionCardConfig

  private _data = new WsDataController<DashboardSummary>(this, (h) =>
    getDashboardSummary(h)
  )

  setConfig(config: ConsumptionCardConfig): void {
...
customElements.define('eon-next-fork-consumption-card', EonNextConsumptionCard)
